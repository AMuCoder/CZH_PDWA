...
#import "WJQIAN.h"
@interface CtrlCodeScan ()<UIGestureRecognizerDelegate>
...
- (void)initScan
{
    ...
    if(authStatus == AVAuthorizationStatusRestricted || authStatus == AVAuthorizationStatusDenied){
        ...
        UIAlertView *alert =[[UIAlertView alloc]initWithTitle:@"网警提示" message:@"请在iPhone的“设置”-“隐私”-“相机”功能中，打开相机访问权限" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles: nil];
...
//    CGRect rect = output.rectOfInterest;
    ...
    label.text = @"对准二维码到框内即可扫描";//@"二维码: 将二维码置于灰色方形区域内; 条形码: 将条形码置于灰色方形区域内并将红线置于条形码竖直中心位置";
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection
{
    ...
    NSString *strUrl = [stringValue stringByReplacingOccurrencesOfString:@" " withString:@"+"];
    [output setMetadataObjectsDelegate:nil queue:nil];
    UserInfo *info = [UserInfoTool info];
    NSMutableDictionary *params = [NSMutableDictionary dictionary];
    params[@"qrcodeStr"] = strUrl;
    params[@"policeName"] = info.UserName;
    [[HttpRequestManager sharedManager]POST:[NSString stringWithFormat:@"%@/%@",CZHURL,@"/police_SignIn"] params:params success:^(NSData *data) {
        NSDictionary *dic = [NSJSONSerialization JSONObjectWithData:data options:NSJSONReadingMutableContainers error:nil];
        //NSString *message = dic[@"return"][@"message"];
        
        WJQIAN *user = [WJQIAN objectWithKeyValues:dic[@"return"]];
